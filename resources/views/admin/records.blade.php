...
<form method="GET" class="mt-5 grid gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:grid-cols-2 lg:grid-cols-6">
    <input class="field" type="date" name="date" value="{{ request('date') }}" aria-label="Ngày">
...
<div class="mt-5 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
    <table class="w-full min-w-[760px] text-left"><thead class="bg-slate-50 text-xs uppercase text-slate-500"><tr><th class="p-4">AP</th><th class="p-4">Tổ</th><th class="p-4">Trạng thái</th><th class="p-4">Thời gian</th><th class="p-4 text-right">Thao tác</th></tr></thead>
    <tbody>@forelse($records as $record)<tr class="border-t border-slate-100"><td class="p-4 font-black">{{ $record->ap_name }}</td><td class="p-4">{{ $record->team?->name ?? '—' }}</td><td class="p-4"><span class="badge {{ $record->status }}">{{ $record->status === 'installed' ? 'Đã lắp' : $record->issue_reason }}</span></td><td class="p-4 text-sm">{{ $record->created_at->format('d/m/Y H:i') }}</td><td class="p-4 text-right"><a class="font-bold text-blue-700" href="{{ route('admin.records.show', $record) }}">Xem</a><a class="ml-4 font-bold text-slate-700" href="{{ route('admin.records.edit', $record) }}">Sửa</a></td></tr>@empty<tr><td colspan="5" class="p-10 text-center text-slate-500">Không có dữ liệu phù hợp.</td></tr>@endforelse</tbody></table>
</div>
